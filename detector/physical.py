"""Физическая проверка типа по измеренным габаритам.

Комплекс знает реальный масштаб пикселя, поэтому у каждой детекции есть длина и ширина
в метрах. Это даёт независимую от нейросети проверку: самолёт заявленного типа не может
быть вдвое меньше паспортного размера.

Проверка ловила бы, например, такое: на гражданском аэродроме модель MAR20 выдала C-5 Galaxy
с измеренными габаритами 38.8 × 35.6 м при паспортных 75.5 × 67.9 м — промах вдвое, физически
невозможный для этого типа.

**Чего проверка не делает.** Совпадение размеров не доказывает тип: гражданский Boeing 767
(48.5 × 47.6 м) по габаритам неотличим от E-8 J-STARS (46.6 × 44.4 м), построенного на базе
Boeing 707. Проверка отсеивает грубые промахи и не заменяет оператора — в MAR20 нет
гражданских классов, поэтому на гражданском аэродроме любой борт получит военный тип.

Источник эталонов — открытые справочные данные по типам (длина фюзеляжа × размах крыла).
"""
