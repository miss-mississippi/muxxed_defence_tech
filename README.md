...
**Важная оговорка про шум.** Устойчивость к сенсорному шуму даёт не размер модели, а аугментация
при обучении: `s`, обученная с шумом, держит σ=15 без потерь (0.900) при той же скорости, тогда
как нынешняя базовая `mar20_s_800.pt` на таком снимке рассыпается (0.371, а при σ=30 — 0.063).
Полные таблицы — [docs/training/robustness.md](docs/training/robustness.md). Правильный кандидат
в поставку по умолчанию — `s` с шумом; её веса пока лежат только в выходе обучающего ноутбука.
