...
На зашумлённом снимке она теряет 1.9% mAP50 там, где базовая теряет 93%
([замер](docs/training/robustness.md)), но сцена обрабатывается дольше: 19.0 с против 13.4 с на
демо-аэродроме (4096×4096, 25 тайлов, M1/MPS, обе модели). Поэтому по умолчанию поставляется `s`,
а `m` — для архивных и заведомо шумных снимков. Веса в git не входят (`weights/` в `.gitignore`):
они получаются из выхода `notebooks/kaggle_train.ipynb` с `MODEL=yolo11m-obb.pt` и `NOISE=0.5`.
